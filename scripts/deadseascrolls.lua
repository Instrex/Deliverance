local dssmod = RegisterMod("Dead Sea Scrolls (Deliverance)", 1)
local game = Game()
...
-- Deliverance menu code starts here!
function MenuProvider.SaveSaveData()
    deliveranceDataHandler.directSave()
end

function MenuProvider.GetPaletteSetting()
    return deliveranceData.persistent.MenuPalette
end

function MenuProvider.SavePaletteSetting(var)
    deliveranceData.persistent.MenuPalette = var
end

function MenuProvider.GetHudOffsetSetting()
    return deliveranceData.persistent.HudOffset
end

function MenuProvider.SaveHudOffsetSetting(var)
    deliveranceData.persistent.HudOffset = var
end

function MenuProvider.GetGamepadToggleSetting()
    return deliveranceData.persistent.MenuControllerToggle
end

function MenuProvider.SaveGamepadToggleSetting(var)
    deliveranceData.persistent.MenuControllerToggle = var
end

function MenuProvider.GetMenuKeybindSetting()
    return deliveranceData.persistent.MenuKeybind
end

function MenuProvider.SaveMenuKeybindSetting(var)
   deliveranceData.persistent.MenuKeybind = var
end

function MenuProvider.GetMenusNotified()
    return deliveranceData.persistent.MenusNotified
end

function MenuProvider.SaveMenusNotified(var)
    deliveranceData.persistent.MenusNotified = var
end

function MenuProvider.GetMenusPoppedUp()
    return deliveranceData.persistent.MenusPoppedUp
end

function MenuProvider.SaveMenusPoppedUp(var)
    deliveranceData.persistent.MenusPoppedUp = var
end
...
if not dssmenu or (dssmenu.CoreVersion < dssCoreIncluded) then
    ...
    dssmenu.CoreMod = "Deliverance"
    DeadSeaScrollsMenu = dssmenu
end

function MenuProvider.IsMenuCore()
    return DeadSeaScrollsMenu.CoreMod == "Deliverance"
end
...
local deliverancedirectory = {
	...
	credits = {
        ...
        fsize = 1,
        buttons = {
            {str = "deliverance team", nosel = true}
        }
    },
}

local credits = {
    {"doctor hummer", "coder,artist"},
	{"instrex", "coder"},
	{"baicherra", "artist"},
	{"fly6", "coder"},
...
for _, credit in ipairs(credits) do
    if type(credit) == "string" then
        deliverancedirectory.credits.buttons[#deliverancedirectory.credits.buttons + 1] = {str = credit, nosel = true}
    else
        for i, part in ipairs(credit) do
            if i ~= 1 then
                if i == 2 then
                    ...
                    deliverancedirectory.credits.buttons[#deliverancedirectory.credits.buttons + 1] = button
                else
                    deliverancedirectory.credits.buttons[#deliverancedirectory.credits.buttons + 1] = {strpair = {{str = ''}, {str = part}}, nosel = true}
                end
            end
        end
    end
...
local deliverancedirectorykey = {
    Item = deliverancedirectory.main,
    Main = 'main',
    Idle = false,
    MaskAlpha = 1,
    Settings = {},
...
dssmenu.AddMenu("Deliverance", {Run = dssmod.runMenu, Open = dssmod.openMenu, Close = dssmod.closeMenu, Directory = deliverancedirectory, DirectoryKey = deliverancedirectorykey})
