...
function this.trigger(id,dmgAmount,flags,source,frames)
  ...
  if player:HasTrinket(this.id) then
    if utils.chancep(6) and flags ~= DamageFlag.DAMAGE_NO_PENALTIES then
      ...
      game:StartRoomTransition(level:QueryRoomTypeIndex(RoomType.ROOM_DEVIL, true, RNG()), Direction.NO_DIRECTION, "3") --поменять на другую команду чтобы не было ангельских сделок
