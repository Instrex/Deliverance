     ...
     if player:HasCollectible(this.id) then
       if room:IsFirstVisit() and not room:IsClear() then
         ...
         for _, entity in pairs(Isaac.GetRoomEntities()) do
           if entity:IsActiveEnemy() and entity:IsVulnerableEnemy() and not entity:HasEntityFlags(EntityFlag.FLAG_FRIENDLY) and not entity:IsBoss() and not this.blacklist[entity.Type] then
...
function this:dropPennies(player)
   local data = player:GetData()
   data.pennyDrop = data.pennyDrop or player:GetCollectibleNum(this.id)
 
   if data.pennyDrop < player:GetCollectibleNum(this.id) then
     for i=1, 5 do
       Isaac.Spawn(5, 20, 1, player.Position, Vector.FromAngle(math.random(0, 360)):Resized(2), player)
     end
     data.pennyDrop = player:GetCollectibleNum(this.id)
   end
 end
...
function this.Init()
  ...
  mod:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, this.dropPennies)
end
