    ...
    if pickup and not pickup:IsShopItem() then
      if pickup:CanReroll() then
        ...
        elseif pickup.Variant==53 then --eternal chest
          ...
          this.divide(pickup)
         elseif pickup.Variant == 300 and game:GetItemPool():GetCard(game.TimeCounter,false,true,true) then
            for i=1, 2 do
               Isaac.Spawn(5, 300, 55, pickup.Position, Vector.FromAngle(math.random(0, 360)):Resized(1), pickup)
            end
            this.divide(pickup)
