...
function this:updatetarget(s)
  ...
  local player = s.Parent or Isaac.GetPlayer()
  ...
  if data.timer2 == 8 or data.timer2 == 16 or data.timer2 == 24 or data.timer2 == 32 or data.timer2 == 40 then
     Isaac.Spawn(1000, airStrike, 0, s.Position + Vector(-240+data.timer2*10, 0), vectorZero, s)
...
function this.use()
  ...
  Isaac.Spawn(1000, airStrike_target, 0, player.Position, vectorZero, player).Parent = player
  sfx:Play(Isaac.GetSoundIdByName("Bomber"), 0.8, 0, false, 1) 
  end

function this.Init()
  mod:AddCallback(ModCallbacks.MC_USE_ITEM, this.use, this.id)
  mod:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, this.updatetarget, airStrike_target)
  mod:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, this.updatemissile, airStrike)
end
