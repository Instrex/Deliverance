...
function this:cache(player, flag)
  ...
  if player:HasCollectible(this.id) then
    if not data.temporary.hasGasoline then
      data.temporary.hasGasoline = true
...
function this:onHitNPC(npc)
  ...
  if not npc:IsBoss() then 
    if player:HasCollectible(this.id) then
      ...
      fire.SpriteScale = Vector(0.5+npc.MaxHitPoints / 50,0.5+npc.MaxHitPoints / 50)
...
function this:updateFire(npc)
  if npc.Variant == this.variant then
    ...
    if data.time >= data.outTime then sprite:Play("End") end
