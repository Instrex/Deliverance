...
function this.use()
  ...
  Isaac.Spawn(1000, specialDel_target, 0, player.Position, vectorZero, nil).Parent = player
end

function this.Init()
  mod:AddCallback(ModCallbacks.MC_USE_ITEM, this.use, this.id)
  mod:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, this.updatetarget, specialDel_target)
  mod:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, this.updatebox, specialDel)
end
